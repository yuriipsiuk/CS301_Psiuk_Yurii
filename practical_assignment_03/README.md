Репозиторій містить розв'язання третьої практичної роботи з CS301, виконаної Юрієм Псюком - практична група 4.
У даному репозиторії місяться:
...
Task 1
 function calculate_order_total(p_order_id int) фільтрує order_items по order_id=p_order_id,
 знаходить їх суму і повертає результата

 Task 2
 procedure create_order(p_customer_id int) перевіряє чи є такий id в customer, 
 якщо є, то створює нове замовлення з поки 0 total_amount 

 Task 3
 procedure add_product_to_order(p_order_id int, p_product_id int, p_quantity int)  перевіряє чи існують order і product 
 з даними id, перевіряє чи достатньо stock_quantity. Якщо всі умови задовольняються, то оновлюється кількість і додається новий рядок в order_items

 Task 4
 trigger update_order_total - справцьовує після DML операцій над order_items і викликає для таких рядків функцію normalizer()
 ...
 Task 5
 trigger logger після створення нових замовлень, викликає для новостоврених рядків в orders add_log(). 
 Дана функція вставляє в order_log рядок з даними про customer_id, order_id і час новоствореного замовлення.
